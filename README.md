...
This repository is a demonstrable local/demo system. The dashboard labels live host metrics, controlled demo telemetry, static topology, and synthetic service-health data separately.
...
- Supports incident history, statistics, search, and status updates
- Similar-incident lookup is root-cause text matching; it does not fabricate a historical match when PostgreSQL is unavailable
...
Metrics Generator (live psutil / demo scenarios)
  ...
  Action Selector / What-If Simulator
        │
        ▼
  React dashboard → FastAPI → ML/AIOps pipeline → PostgreSQL incident persistence
```
...
- Detection: Isolation Forest is combined with weighted rules/hybrid scoring.
- Diagnosis: root cause is selected through weighted contribution analysis.
- Prediction: failure forecasting is trend-informed from the current metric window.
- Decision: recommendations are generated by a deterministic engine; what-if actions are simulations only.
- Incidents: PostgreSQL stores incidents and powers history, statistics, search, and status update APIs. A database failure returns a clear API error rather than empty incident data.
- Topology: the service map is a static model topology. Its displayed per-service health is synthetic, and cascade impact is explicitly a simulation.
...
- No LLM assistant, automatic production remediation, real distributed tracing, or real service discovery
- No per-service production telemetry; topology health is synthetic
