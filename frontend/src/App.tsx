...
const ProtectedRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  if (!isInitialized) return <Navigate to="/init" replace />;
  return <>{children}</>;
};
...
const AuthRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { isAuthenticated, isInitialized } = useAuth();
  if (isAuthenticated && isInitialized) return <Navigate to="/dashboard" replace />;
  if (isAuthenticated && !isInitialized) return <Navigate to="/init" replace />;
  return <>{children}</>;
};
