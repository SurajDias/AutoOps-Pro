...
function createStackedPlanesBufferGeometry(n: number, width: number, height: number, spacing: number, heightSegments: number) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      uvs.set([uvXOffset, uvY + uvYOffset, AppendingUV(uvXOffset, 1), uvY + uvYOffset], uvOffset);
  ...
  function AppendingUV(base: number, val: number) {
    return base + val;
  }
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  const geometry = useMemo(
    () => createStackedPlanesBufferGeometry(count, width, height, 0.4, 60),
    [count, width, height]
  );
  useFrame((_, delta) => {
    if (mesh.current) {
